...
### Libraries
To start with, we need to load the required libraries. For random forest we'll be using the ranger package. Then we'll use ggplot2 to plot the results, with the fanctastic theme provided by the director of data science at Airbnb. 
```{r libraries}
library(ranger)
library(tidyverse)
library(ggplot2)
...
To test this, we prepared an artificial dataset. We try to predict the default probabability as a factor of the type of employment. The two options are: salary and self employed. The analisys is based on historical data and such we have an imbalaced set: 10 000 people on salary, and only 500 self empolyed.   
...
In the previus experiment, the default probability was very different fo the two groups. To exclude the possibility of this infuencing the variance, we prepared a second experiment. This time the groups are more or less even, but the default rate is different.   
...
And the results are also promising - the groups seem to have simmilar margins of confidence, despite having very different probasbilities of default. 
...
To push the experiments further, we searched for a real dataset. We found a promising datbase from Taiwan, consisting of 30 000 observations and 23 variables.  
